...
Reference fixtures generated by running nvdiffrast on CUDA and a Swift-side
parity test bench live in `Tests/MLXDiffRastTests/`. Results:

- **`texture` (bilinear & trilinear): bit-exact match** with nvdiffrast (max
  abs diff ≈ 6×10⁻⁸ — pure FP rounding).
- **`rasterize` / `interpolate`: differ by two documented conventions** — see
  [`CONVENTIONS.md`](CONVENTIONS.md). Both are valid; both libraries are
  internally consistent. The differences are the image-y orientation
  (top-left vs OpenGL bottom-left) and the barycentric channel layout
  (`rast.u = w₁` vs `rast.u = w₀`). Closeable in 1–2 kernel lines if strict
  nvdiffrast compatibility is needed.

To regenerate fixtures (requires a CUDA host with the nvdiffrast PyTorch
bindings): `python scripts/diffrast_fixtures.py --out <path>.safetensors`.
